...
const Dashboard = () => {
  ...
  const periodoPadrao = useMemo(() => getPeriodoAtual(), []);
  ...
  const handleEdit = (t) => {
    ...
    // Navegação suave para a âncora
    document.getElementById('novaTransacao')?.scrollIntoView({ behavior: 'smooth' });
  };
  ...
  return (
    ...
    >
      <div className="w-full max-w-300 mx-auto p-4 sm:p-8 flex-1">
        ...
        {/* --- BARRA DE FILTROS --- */}
        <div 
          className="grid grid-cols-1 md:grid-cols-4 gap-4 p-5 rounded-2xl border shadow-sm mb-8 transition-all items-center"
        ...
        >
          <div className="relative">
            ...
            <input 
              type="date" 
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border bg-transparent outline-none transition-all"
              style={{ borderColor: theme.border, color: theme.text }}
              value={filtros.data_inicio}
              onChange={(e) => setFiltros({ ...filtros, data_inicio: e.target.value })}
            />
            <span className="absolute -top-2 left-3 px-1 text-[10px] uppercase font-bold" style={{ backgroundColor: theme.surface, color: theme.text }}>Início</span>
          </div>
          ...
          <div className="relative">
            ...
            />
            <span className="absolute -top-2 left-3 px-1 text-[10px] uppercase font-bold" style={{ backgroundColor: theme.surface, color: theme.text }}>Fim</span>
          </div>
          ...
          <div className="relative">
            ...
            />
            <span className="absolute -top-2 left-3 px-1 text-[10px] uppercase font-bold" style={{ backgroundColor: theme.surface, color: theme.text }}>Pesquisar</span>
          </div>

          <button 
            onClick={limparFiltros}
            className="flex items-center justify-center gap-2 py-2.5 rounded-xl border font-medium transition-all hover:opacity-80 active:scale-95"
            style={{ 
